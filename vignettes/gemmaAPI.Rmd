...
The Gemma API wrapper is very straightforward to use and contains a function for each entry in the REST API docs, [here](https://gemma.msl.ubc.ca/resources/restapidocs/). We suggest you make some example queries on the interactive API docs to get a sense of the inputs and outputs.

In this example, we want to find datasets in Gemma that are associated with bipolar disorder, and we are only interested in human data. 
To do this, we can use the `searchDatasets()` endpoint, and we sort the resulting datasets according to their [Gemma Quality Scores](https://pavlidislab.github.io/Gemma/geeq.html):
...
The first endpoint in the REST API doc corresponds to a simple case: getting basic information about a single dataset in Gemma. We can do this in one of two ways using the API:

1.  Using the dataset's short name (GSE35974), or
2.  Using the dataset's identifier in Gemma (e.g `1`, `2`, ...)

Each case is functionally identical in the API wrapper.

```{r}
glimpse(getDatasets(datasets = 5939))

glimpse(getDatasets(datasets = 'GSE35974'))
```

## Bioconductor data structures
The function `getBioconductor()` is a high-level wrapper that combines various 
endpoints to return an annotated SummarizedExperiment or ExpressionSet of the queried dataset that is compatible with other Bioconductor packages. Both classes are very similar, SummarizedExperiments are newer and more suitable for RNA-Seq data. Read the Summarized
Experiment [vignette](https://bioconductor.org/packages/release/bioc/vignettes/SummarizedExperiment/inst/doc/SummarizedExperiment.html) or the ExpressionSet [vignette](https://bioconductor.org/packages/release/bioc/vignettes/Biobase/inst/doc/ExpressionSetIntroduction.pdf) for more details.
```{r}
getBioconductor('SummarizedExperiment', 'GSE35974', filter = TRUE)
getBioconductor('ExpressionSet', 'GSE35974', filter = TRUE)
```
...
As you can see, the asynchronous method is much faster (about 6x), even on this small test. For larger queries, the gains are even larger, such as if we wanted to fetch all annotations in Gemma.
...
synchronise({
  ...
  async_map(split(1:20000, ceiling(seq_along(1:20000) / 1000)), async(function(chunk) {
    ...
    }), .limit = 50) # Only allow 50 active requests at a time to not flood the server
  })
}) %>% rbindlist %>% glimpse
```
...
```{r}
invisible(getDatasetSVD('GSE35974', raw = TRUE, file = 'SVD')) # Saves SVD.json like above, but suppresses output in R
```
