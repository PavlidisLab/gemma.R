...
endpoints to return an annotated SummarizedExperiment or ExpressionSet of the queried dataset that is compatible with other Bioconductor packages for downstream analyses. 
Both classes are very similar, SummarizedExperiments are newer and more suitable for RNA-Seq data.
They include the expression matrix along with
...
We can quickly see three samples among bipolar patients and controls are empty. This is because during the data curation process Gemma flags potential outlier samples automatically, which are then manually reviewed. Samples confirmed to be outliers are represented as missing data to ensure transparency.

Check the Summarized  Experiment [vignette](https://bioconductor.org/packages/release/bioc/vignettes/SummarizedExperiment/inst/doc/SummarizedExperiment.html) or the ExpressionSet [vignette](https://bioconductor.org/packages/release/bioc/vignettes/Biobase/inst/doc/ExpressionSetIntroduction.pdf) for more details on how to use/modify these data structure.
...
We included another wrapper, `getTidyDataset()` to get data in a "long" format also combines the design and expression matrices and ensures they always match. This format allows visualization with the popular [ggplot2](https://ggplot2.tidyverse.org/) package and is compatible with the rest of the [tidyverse](https://www.tidyverse.org/).
This "long" format also combines the design and expression matrices and ensures they always match. 
...
For very large requests, this is often insufficient and instead, we recommend using asynchronous requests, which are explained in the async vignette.
