...
This is made more obvious by example. Take the previous instance where we wanted to fetch annotations for two datasets, `GSE2018` and `GSE25136`. If we want to run API queries asynchronously, we only need to:
1.  Embed our requests inside a `synchronise` block
2.  Set `async = TRUE` in our requests
...
For completeness, we'll provide a small benchmark to hopefully convince you that asynchronous programming is worth learning.
...
knitr::kable(microbenchmark(asyncBenchmark(), syncBenchmark(), times = 10))
```
...
```{r}
system.time(getDatasetData('GSE2018', memoised = T))
system.time(getDatasetData('GSE2018', memoised = T))
```
