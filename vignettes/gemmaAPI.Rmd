...
We are left with two datasets, and we'll pick [GSE35974](https://gemma.msl.ubc.ca/expressionExperiment/showExpressionExperiment.html?id=5939) since it has the larger sample size.
...
Finally, you can use ['getTidyDataset()'](https://pavlidislab.github.io/Gemma-API/reference/getTidyDataset.html) to fetch data in a long, 'tidy' format that combines the expression matrix with the metadata. This is less memory-efficient than the two alternatives above, but allows quick and easy visualization and exploration of the data with [`ggplot2`]https://ggplot2.tidyverse.org/).
...
```{r outputs}
getDatasetAnnotations('GSE35974', raw = TRUE, file = 'SVD') %>% glimpse() # raw = T, saves SVD.json
