...
Expression data in Gemma comes with annotations for the gene each expression profile corresponds to. Using the [`get_platform_annotations()`](https://pavlidislab.github.io/gemma.R/reference/get_platform_annotations.html) function, these annotations can be retrieved independently of the expression data, along with additional annotations such as Gene Ontology terms.

Examples:

```{r}
get_platform_annotations('GPL96')
```

```{r}
get_platform_annotations('Generic_human')
```
...
get_gene_probes(2026)
```

## Differential expression analyses

Gemma contains precomputed differential expression analyses for
most of its datasets. Analyses can involve more than one factor, such as "sex" as well as "disease". ome datasets contain more than one analysis
to account for different factors and their interactions. The results are stored as resultSets, each corresponding to one factor (or their interaction). You can access them using
