...
as well as "disease". ome datasets contain more than one analysis to
...
Note that `get_differential_expression_values()` has the argument `readableContrasts`.
By default this is set to `FALSE`, resulting in the output columns being named based
on the contrast IDs rather the more human readable columns used below. These contrast
IDs can be used together with [`get_dataset_differential_expression_analyses()`](https://pavlidislab.github.io/gemma.R/reference/get_dataset_differential_expression_analyses.html) during a systematic analysis.
...
Some endpoints accept multiple identifiers in a single function
...
lapply(c("GSE35974", "GSE12649"), function(dataset) {
    get_dataset_annotations(dataset) %>% 
        ...
        select(experiment.shortName, class.Type, class.Name, term.Name)
}) %>% do.call(rbind,.)
```
