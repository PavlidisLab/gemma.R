...
searchDatasets("bipolar", limit = 100, taxon = "human") %>%
    ...
    select(ee.ShortName, ee.Name, ee.ID, ee.Accession, ee.Database, ee.Samples)
```
...
To get the basic information of a dataset, we use the [`getDatasetsInfo()`](https://pavlidislab.github.io/gemma.R/reference/getDatasetsInfo.html) endpoint.

```{r dataset}
getDatasetsInfo("GSE35974") %>%
    select(ee.ShortName, ee.Name, ee.ID, ee.Accession, ee.Database, ee.Samples)
```
...
Alternatively, you can use [`getDatasetExpression()`](https://pavlidislab.github.io/gemma.R/reference/getDatasetExpression.html) to only get the expression matrix, and [`getDatasetDesign()`](https://pavlidislab.github.io/gemma.R/reference/getDatasetDesign.html) to get a the experimental design matrix. Finally, you can use [`getTidyDataset`](https://pavlidislab.github.io/gemma.R/reference/getTidyDataset.html) to get a tibble that combines the expression and design matrices in tidy format. This format is less memory efficient than [`getDataset()`](https://pavlidislab.github.io/gemma.R/reference/getDataset.html) but allows easy visualization and exploration with [ggplot2](https://ggplot2.tidyverse.org/index.html) and the rest of the [tidyverse](https://www.tidyverse.org/).
...
dat$DiseaseState %>% unique()

# Subset patients with bipolar disorder and controls.
bipolar <- dat[, dat$DiseaseState == "Bipolar_Disorder" | dat$DiseaseState == "reference_subject_role"]
bipolar
```
...
Gemma also contains differential expression results for most of its datasets, and some datasets contain more than one analysis to account for different factors and their interactions. These tables are stored as resultSets, and you can access them using [`getDatasetDE()`](https://pavlidislab.github.io/gemma.R/reference/getDatasetDE.html). From here on, you can visualize/explore the data and find the most differentially-expressed genes:
...
filter(de, diffexpr == "Up") %>%
    ...
    select(Probe, GeneSymbol, `contrast_Bipolar Disorder_pvalue`, 
        `contrast_Bipolar Disorder_logFoldChange`)
...
ggplot(
    ...
    geom_vline(xintercept = c(-0.4, 0.4), col = "grey", linetype = "dashed") +
...
```
In this case, you can check the available resultSets to get the ID you want and
or use `all = TRUE` to get a list containing all the resultSets.
```{r}
# Show available resultSets
...
### Larger queries
The `*Info()` endpoints accept multiple identifiers in a single function call For example, getting information on 3 datasets at the same time.

```{r triple-query}
getDatasetsInfo(datasets = c("GSE35974", "GSE46416", "GSE99349")) %>%
...
the `limit` and `offset` arguments. To avoid overloading the system, calls are limited
to a maximum of 100 entries, so the offset allows you to get the next batch of entries
in the next call(s).
```{r}
getDatasetsInfo(limit = 5) %>% select(ee.ID, ee.ShortName)
getDatasetsInfo(limit = 5, offset = 5) %>% select(ee.ID, ee.ShortName) 
getDatasetsInfo(limit = 5, offset = 10) %>% select(ee.ID, ee.ShortName) 
```
