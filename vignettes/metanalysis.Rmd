...
example we will use a very simple approach, [Fisher's combined p-value test](https://en.wikipedia.org/wiki/Fisher%27s_method). 
This is implemented in the `fisher` function from `poolr` package.
...
relevant datasets using `get_dataset_expression_for_genes`. We provide the IDs of the experiments and the NCBI ID of the gene.
...
Then we grab the metadata for the samples so we can organize the data by disease condition vs. control:
